...
@Component({
	selector: "app-historial-estado-solicitudes",
	templateUrl: "./historial-estado-solicitudes.component.html",
	styleUrls: ["./historial-estado-solicitudes.component.css"],
...
export class HistorialEstadoSolicitudesComponent implements OnInit, OnDestroy {
	...
	createTable(
		...
		this.estadosOperacion.forEach((estado: string) => {
			...
			let row = this.createRow(array, estado);
			array = [];
			objetos.push(row);
		});
		this.loading = false;
	...
	createRow(array: Array<number>, estadoOperacion: string): Object {
		let estado = estadoOperacion.toLowerCase();
		let object = {
			estadoOperacion: estado.charAt(0).toUpperCase() + estado.slice(1),
			mes1: array[0],
			mes2: array[1],
			mes3: array[2],
			mes4: array[3],
