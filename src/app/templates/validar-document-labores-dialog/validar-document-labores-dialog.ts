...
@Component({
   ...
   templateUrl: './validar-document-labores-dialog.html',
})
export class ValidardocumentLarboresDialog implements OnInit {
   ...
   constructor(
      public dialogRef: MatDialogRef<ValidardocumentLarboresDialog>,
   ...
   ngOnInit() {
      ...
      this.data['estados'].forEach((estado, index) => {
         // if (estado.nombre!="Pendiente"||estado.idestadodocumento!=1 ){
      ...
      this.estadoSelected = this.data['labor']['idestadodocumento']
         ...
         (respont) => {
            if (respont['code'] == 200 && respont['data'].length) {
               respont['data'].forEach(estadoPendiente => {
                  this.estadoSelectedPendiente.push(estadoPendiente.idestadodocumento);
               });;
            }
            this.cargando = false;
